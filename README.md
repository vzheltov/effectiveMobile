...
Backend-приложение на Django REST Framework с собственной логикой:

- регистрации и входа по email и паролю;
- bcrypt-хеширования паролей;
- JWT-аутентификации;
- серверного logout через отзыв сессии;
- мягкого удаления аккаунта;
- ролевого доступа к ресурсам с разделением прав `own` и `all`;
- API администратора для изменения ролей, ресурсов и правил;
- mock-ресурсов для демонстрации работы авторизации.

## Аутентификация и авторизация

**Аутентификация** отвечает на вопрос «кто выполняет запрос». После login сервер
...
Logout заполняет `revoked_at`, поэтому тот же JWT сразу перестаёт работать.
Soft delete дополнительно отзывает все активные сессии пользователя.
...
Неизвестные действия безопасно отклоняются. Роль с кодом `admin` является
bootstrap-правилом для API управления доступом. Последнюю активную admin-роль
нельзя удалить или снять с пользователя.
...
Без переменной `POSTGRES_HOST` используется SQLite. Это удобно для быстрых
локальных тестов; Docker-конфигурация использует PostgreSQL.
...
Demo-учётные записи предназначены только для локальной демонстрации:
